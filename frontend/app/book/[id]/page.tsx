...
export default function BookingDetails() {
  ...
  useEffect(() => {
    axios
      .get(`${bookingsAPIURL}/booking/${bookingId}`)
      .then((response) => {
  ...
  useEffect(() => {
    if (booking) {
      setPosition([
        booking.source.coordinates[1],
        booking.source.coordinates[0],
      ]);
    }
  }, [booking]);
  ...
  useEffect(() => {
    ...
    const fetchLocation = () => {
      axios
        .get(`${bookingsAPIURL}/driver?id=${booking.driver_id}`)
        .then((response) => {
  ...
  useEffect(() => {
    ...
    const socket = new WebSocket(
      `${bookingsAPIWS}/ws/notification?id=${userId}`
    );
        ...
        axios
          ...
          .then((response) => {
            ...
            setCurrentStatusIndex(
              currentStatuses.indexOf(response.data.status)
            );
          })
          .catch((error) => {
            console.error(error);
    ...
    if (currentStatusIndex < currentStatuses.length - 1) {
      ...
      if (notificationWs && booking) {
        const jsonData = JSON.stringify({
          ...
          status: currentStatuses[currentStatusIndex + 1],
        ...
        <div className="p-6 min-h-screen">
          ...
          {booking ? (
            <div className="bg-white p-6 rounded-lg shadow-md mb-4">
              ...
              <p className="text-gray-700">
                <span className="font-semibold">Status:</span> {currentStatuses[currentStatusIndex]}
              </p>
              <p className="text-gray-700">
          ...
          {booking && (
            ...
            >
              ...
              >
                <Tooltip permanent>
                  Source Location: {booking.source.coordinates[0]},{" "}
                  {booking.source.coordinates[1]}
                </Tooltip>
              </Marker>
              {/* Destination Marker */}
              <Marker
                position={[
                  booking.destination.coordinates[1],
                  booking.destination.coordinates[0],
                ]}
              >
                <Tooltip permanent>
                  Destination Location: {booking.destination.coordinates[0]},{" "}
                  {booking.destination.coordinates[1]}
                </Tooltip>
              </Marker>
              {/* Car Marker */}
